...
const checkIfServiceExist = (contacts, service, username) => {
    ...
    MongoClient.connect(url, { useUnifiedTopology: true }, function (err, client) {
        ...
        checkIfServiceExist(contacts, service, username)
            .then(res => {
                ...
                res ? argsOpt.event.emit('Y/N', { collection: contacts, modifiedService: res })
                    : contacts.insertOne({ //if does not exist we save it 
                        ...
                        if (err) console.log(err)
                        console.log(' Success, new service registered :: ', doc.ops[0].service, doc.ops[0].name)
...
exports.getServicefromOptionArgs = (argsOpt) => {
    ...
    MongoClient.connect(url, { useUnifiedTopology: true }, function (err, client) {
        ...
        db.collection(collection).findOne({ service: service }, { projection: { _id: 0 } })
            .then(res => {
                if (!res) {
                    process.stderr.write('no service found\n')
                    argsOpt.event.emit('close')
                }
                console.log(style.colors.yellow + decrypt(res.password) + style.reset)
                argsOpt.event.emit('close')
            })
